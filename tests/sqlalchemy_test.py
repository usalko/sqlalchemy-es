...
from sqlalchemy_dialects.es.exceptions import DatabaseError
from sqlalchemy_dialects.es.sqlalchemy import ES_HTTP_DIALECT

from tests.fixtures.fixtures import data1_columns, flights_columns
...
class TestSQLAlchemy(unittest.TestCase):
    def setUp(self):
        registry.register('es.http', ES_HTTP_DIALECT.__module__, ES_HTTP_DIALECT.__name__)

        self.driver_name = os.environ.get('ES_DRIVER', 'es')
        ...
        uri = URL(
            ...
            port,
            None,
            {'verify_certs': str(verify_certs), 'v2': f'{self.v2}'},
        )
        self.engine = create_engine(uri)
        self.connection = self.engine.connect()
        # FIXME: Autocreate index
